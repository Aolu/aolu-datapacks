...
execute if score @s aolu_snt_lifetime matches 2 store result storage aolu_snt:spell_damage a double 1.4 at @a if score @s cr.owner_uuid_sum = @p cr.owner_uuid_sum run attribute @p attack_damage get
execute if score @s[tag=aolu_snt_weak_spike] aolu_snt_lifetime matches 2 store result storage aolu_snt:spell_damage a double 0.004 at @a if score @s cr.owner_uuid_sum = @p cr.owner_uuid_sum run attribute @p attack_damage get 100
execute if score @s aolu_snt_lifetime matches 2 as @a if score @s cr.owner_uuid_sum = @n cr.owner_uuid_sum at @e[distance=.1..3,type=!#aolu_staffs_n_tomes:ignore] unless score @s cr.owner_uuid_sum = @n cr.owner_uuid_sum run function aolu_staffs_n_tomes:spells/amethyst/spike_damage with storage aolu_snt:spell_damage
