...
execute at @e[distance=..8,limit=5,type=!#aolu_staffs_n_tomes:ignore] unless score @n cr.owner_uuid_sum = @s cr.owner_uuid_sum run summon armor_stand ~ ~ ~ {Marker:1b,Silent:1b,Invulnerable:1b,Small:1b,Invisible:1b,NoBasePlate:1b,Tags:["aolu_snt_spell","aolu_snt_geyser","aolu_snt_proj"],DisabledSlots:4144959}
scoreboard players operation @e[tag=aolu_snt_proj] cr.owner_uuid_sum = @s cr.owner_uuid_sum
