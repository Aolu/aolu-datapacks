...
execute as @a if score @n[tag=aolu_snt_dark_mass] aolu_cr_pid = @s aolu_cr_pid positioned ~-.5 ~-.5 ~-.5 if entity @e[tag=!aolu_snt_dark_mass,type=!#aolu_common_resources:ignore,dx=.5,dy=.5,dz=.5] positioned ~.5 ~.5 ~.5 run function aolu_staffs_n_tomes:spells/shadow/dark_mass_detect
