...
execute positioned ~ ~0.5 ~ run summon endermite ^ ^ ^ {CustomName:"Nest Spawn",DeathLootTable:empty,Silent:1b,Tags:["aolu_snt_summon","aolu_snt_nest_spawn","aolu_snt_proj"],active_effects:[{id:"minecraft:invisibility",amplifier:1,duration:-1,show_particles:0b}]}
effect give @n[type=endermite,tag=aolu_snt_proj] fire_resistance infinite
...
attribute @n[type=endermite,tag=aolu_snt_proj] scale modifier add aolu_snt:summon 1 add_multiplied_total
