...
execute as @e[distance=0.1..8,type=!#aolu_staffs_n_tomes:ignore] unless score @s cr.owner_uuid_sum = @n[tag=aolu_snt_nest] cr.owner_uuid_sum run damage @s 0 sonic_boom by @n[tag=aolu_snt_nest]
...
execute as @a if score @n[tag=aolu_snt_nest] cr.owner_uuid_sum = @s cr.owner_uuid_sum run execute store result storage aolu_snt:spell_damage n float 0.2 run attribute @s attack_damage get
