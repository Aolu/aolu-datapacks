...
execute at @s if score #temp aolu_snt_lifetime matches 0 as @e[distance=0.1..10,type=!#aolu_staffs_n_tomes:ignore] unless score @s cr.owner_uuid_sum = @n[tag=aolu_snt_nest_spawn] cr.owner_uuid_sum run damage @n[tag=aolu_snt_nest_spawn] 0.01 out_of_world by @s
